...
///     validate::{Untrusted, Unvalidated, Validate},
/// };
///
...
impl UserSlice {
    ...
    pub fn read_all(self, buf: &mut KVec<Untrusted<u8>>, flags: Flags) -> Result {
...
impl UserSliceReader {
    ...
    ///
    /// Fails with [`EFAULT`] if the read happens on a bad address.
    pub fn read_all(mut self, buf: &mut KVec<Untrusted<u8>>, flags: Flags) -> Result {
        let len = self.length;
