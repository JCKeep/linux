...
/// Maximum number of port regions supported by UIO.
pub const MAX_UIO_PORT_REGIONS: usize = bindings::MAX_UIO_PORT_REGIONS as _;
...
pub trait UioDevice {
    ...
    ///
    /// This type must implement the `ForeignOwnable` trait, which allows it to
    /// be safely converted between foreign (C) and Rust representations, as well
    /// as `Send` and `Sync` for thread-safe usage.
    type Ptr: ForeignOwnable + Send + Sync;

    /// Called when the UIO device is opened.
    ///
    /// This method is invoked when a user-space application opens the file descriptor
    /// associated with the UIO device. Implementors should use this method to perform
    /// device-specific initialization and return a pointer that will be stored as
    /// private data for the device.
    ///
    /// # Parameters
    /// - `_info`: A reference to the `Info` object containing metadata about the UIO device.
    ///
    /// # Returns
    /// - On success, returns a pointer of type `Self::Ptr`.
    /// - On failure, returns an appropriate error.
    fn open(_info: &Info) -> Result<Self::Ptr>;

    /// Called when the UIO device is released.
    ///
    /// This method is invoked when a user-space application closes the file descriptor
    /// associated with the UIO device. Implementors can use this method to clean up any
    /// resources associated with the device. By default, it simply drops the device pointer.
    ///
    /// # Parameters
    /// - `device`: The pointer to the device instance.
    /// - `_info`: A reference to the `Info` object containing metadata about the UIO device.
    ...
    ///
    /// This method can be used to enable or disable interrupts for the UIO device.
    /// By default, this method generates a build-time error if not overridden,
    /// indicating that the functionality is not implemented.
    ///
    /// # Parameters
    /// - `_device`: A borrowed reference to the device instance.
    /// - `_info`: A reference to the `Info` object containing metadata about the UIO device.
    /// - `_irq_on`: Indicates whether to enable (`1`) or disable (`0`) interrupts.
    ///
    /// # Returns
    ...
    ///
    /// This method is invoked when an interrupt is raised for the device. Implementors
    /// can use this method to handle the interrupt appropriately.
    ///
    /// # Parameters
    /// - `_device`: A borrowed reference to the device instance.
    /// - `_info`: A reference to the `Info` object containing metadata about the UIO device.
    /// - `_irq`: The interrupt number being handled.
    ///
    /// # Returns
    /// - The return value must be a valid `irqreturn_t` indicating the status of the interrupt handling.
    fn handler(
        _device: <Self::Ptr as ForeignOwnable>::Borrowed<'_>,
        _info: &Info,
        _irq: ffi::c_int,
    ...
    ///
    /// This method is invoked during the memory mapping process when user space
    /// requests to map a region of the device into its virtual address space.
    /// Implementors can use this method to configure the mapping.
    ///
    /// # Parameters
    /// - `_device`: A borrowed reference to the device instance.
    /// - `_info`: A reference to the `Info` object containing metadata about the UIO device.
    /// - `_vma`: A reference to a `VmAreaNew` object representing the memory area being mapped.
    ///
    /// # Returns
    /// - On success, returns `Ok(())`.
    /// - On failure, returns an appropriate error.
...
unsafe extern "C" fn uio_open<T: UioDevice>(
    ...
    let ptr = match T::open(Info::from_raw(info)) {
        Ok(ptr) => ptr,
...
unsafe extern "C" fn uio_release<T: UioDevice>(
    ...
    T::release(ptr, Info::from_raw(info));
...
unsafe extern "C" fn uio_irqcontrol<T: UioDevice>(
    ...
    match T::irqcontrol(device, Info::from_raw(info), irq_on as _) {
        Ok(_) => 0,
...
unsafe extern "C" fn uio_handler<T: UioDevice>(
    ...
    T::handler(device, Info::from_raw(dev_info), irq)
}
...
unsafe extern "C" fn uio_mmap<T: UioDevice>(
    ...
    match T::mmap(device, Info::from_raw(info), area) {
        Ok(_) => 0,
        Err(err) => err.to_errno(),
    }
}
...
impl Info {
    ...
    ///
    /// # Safety
    /// - The caller must ensure that the pointer is valid for the lifetime of the returned reference.
    /// - Undefined behavior will occur if the pointer is null or points to invalid memory.
    pub fn from_raw<'a>(ptr: *mut bindings::uio_info) -> &'a Self {
        unsafe { &*ptr.cast() }
    }

    /// Notifies the kernel that an event has occurred on the UIO device.
    ///
    /// This corresponds to calling the `uio_event_notify` function in the kernel,
    /// which signals to the kernel that an interrupt or other relevant event
    /// has occurred.
    ///
    /// # Safety
    /// This method involves calling an unsafe kernel function. Ensure that:
    /// - The `uio_info` structure represented by this `Info` instance is properly
    ///   initialized and in a valid state.
    /// - The kernel module managing the UIO device supports `uio_event_notify`.
    #[inline]
    pub fn notify(&self) {
        // SAFETY: Caller must ensure that `self.inner.get()` points to a valid `uio_info`
        // structure, and that calling `uio_event_notify` is safe in the current context.
        unsafe {
...
pub enum MemType {
    /// No memory is mapped. This can be used for devices that do not require memory access.
    None = bindings::UIO_MEM_NONE as _,
    /// Physical memory address mapping. Typically used for mapping device memory directly.
    Physical = bindings::UIO_MEM_PHYS as _,
    /// Logical memory address mapping. Refers to kernel logical addresses
    /// (addresses accessible directly by the kernel). Usually mapped with 
    /// `ioremap`.
    Logical = bindings::UIO_MEM_LOGICAL as _,
    /// Virtual memory address mapping. Refers to memory dynamically allocated 
    /// by the kernel (e.g., with `kmalloc`) and shared with user space.
    Virtual = bindings::UIO_MEM_VIRTUAL as _,
    /// IO virtual address (IOVA) mapping. Used in devices with DMA support and an IOMMU.
    IoVirtual = bindings::UIO_MEM_IOVA as _,
}

/// Types of port mappings for UIO devices.
pub enum PortType {
    /// No port is used for the device.
    None = bindings::UIO_PORT_NONE as _,
    /// x86-specific I/O port mapping. Typically used for legacy devices on x86 platforms.
    X86 = bindings::UIO_PORT_X86 as _,
    /// General-purpose I/O (GPIO) port mapping. Used for devices interfacing through GPIO pins.
    Gpio = bindings::UIO_PORT_GPIO as _,
    /// Other types of port mappings. Represents non-standard or custom port types.
